...
- **UI POLISH:** Resolved all remaining Material 3 deprecation warnings in the Flutter app (replaced `value` with `initialValue` in `DropdownButtonFormField` and `withOpacity` with `withValues`).
