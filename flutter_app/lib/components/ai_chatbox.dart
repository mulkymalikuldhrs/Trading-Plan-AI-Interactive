    ...
    if (_controller.text.isNotEmpty) {
      ...
      try {
        final response = await ApiService.post('getGptFeedback', {
          ...
          'referenceId': 'chat-ref-${DateTime.now().millisecondsSinceEpoch}',
          'promptData': {'last_action': userInput},
        });
        setState(() {
          _messages.removeLast();
          _messages.add({'message': response['root_cause_question'] ?? 'I understand. Tell me more about your process.', 'isUser': false});
        });
      } catch (e) {
        setState(() {
          _messages.removeLast();
