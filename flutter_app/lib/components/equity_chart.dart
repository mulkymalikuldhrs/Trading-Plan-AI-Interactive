  ...
  void _loadData() async {
    try {
      ...
      for (int i = 0; i < data.length; i++) {
        final trade = data[i];
        if (trade['Result'] == 'WIN') {
          currentEquity += 200; // Simplified profit
          wins.add(i + 1);
        } else if (trade['Result'] == 'LOSS') {
          currentEquity -= 100; // Simplified loss
        }
    ...
    return LineChartData(
      ...
      titlesData: FlTitlesData(
          ...
          bottomTitles: SideTitles(
            showTitles: true,
            reservedSize: 22,
            getTextStyles: (c,v) => const TextStyle(color: Colors.white70, fontSize: 10),
            getTitles: (value) => value % 1 == 0 ? 'T${value.toInt()}' : ''
          ),
