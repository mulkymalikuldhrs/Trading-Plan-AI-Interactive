    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        if (feedback['is_valid_setup'] == true) {
           ...
           double sl = double.tryParse(_slController.text) ?? 0;
           double tp = double.tryParse(_tpController.text) ?? 0;
           double rrr = 0;
           if ((sl - entry).abs() > 0) {
             rrr = (tp - entry).abs() / (sl - entry).abs();
           }
           ...
           if (!mounted) return;
           ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Trade Logged Successfully!")));
        }
