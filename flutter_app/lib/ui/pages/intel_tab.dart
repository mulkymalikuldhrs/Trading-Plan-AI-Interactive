...
class _IntelTabState extends State<IntelTab> {
  ...
  final List<String> _symbols = ["EURUSD", "GBPUSD", "USDJPY", "AUDUSD", "XAUUSD"];
  ...
  Widget _buildCotSummary() {
    ...
    return _buildIntelCard(
      ...
      child: ListTile(
        ...
        trailing: Icon(
          bias.toString().toUpperCase() == 'BULLISH'
              ? Icons.arrow_upward
              : (bias.toString().toUpperCase() == 'BEARISH' ? Icons.arrow_downward : Icons.horizontal_rule),
          color: bias.toString().toUpperCase() == 'BULLISH'
              ? Colors.green
              : (bias.toString().toUpperCase() == 'BEARISH' ? Colors.red : Colors.grey),
        ),
      ),
    );
  }
