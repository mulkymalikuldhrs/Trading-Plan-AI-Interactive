...
function getTechnicalIndicators(symbol) {
  ...
  try {
    ...
    if (data.s === 'ok') {
      ...
      Logger.log("Finnhub API Error: " + data.error || "Unknown error");
      return { rsi: 50, status: "error", message: data.error || "Check Finnhub API permissions" };
    }
  } catch (e) {
    Logger.log("Finnhub Fetch Error: " + e.message);
  }
  return { rsi: 50, status: "fallback", message: "Connection to Finnhub failed" };
}
...
/**
 * Fetches upcoming events from an economic calendar using Finnhub.
 */
function getEconomicCalendar() {
  if (!FINNHUB_API_KEY) return [{ event: "API Key missing", impact: "N/A" }];
