...
function doPost(e) {
  ...
  // Security Check
  if (data.apiKey !== BOT_API_KEY) {
    if (action === 'getAiMasterSummary' || action === 'getForecast' || action === 'triggerWeeklyAnalysis') {
       throw new Error("Unauthorized: Invalid API Key for " + action);
    }
  }
