...
function logViolation(data) {
  ...
  // Emotional Lockout Trigger: Check if last 3 violations occurred within the last 24 hours
  const lastRows = violationsSheet.getLastRow();
  const headerRows = 1; // Assuming 1 header row
  if (lastRows >= (headerRows + 3)) {
      const recentViolations = violationsSheet.getRange(lastRows - 2, 2, 3, 1).getValues(); // Column 2 is Timestamp
      const now = new Date().getTime();
      const twentyFourHoursAgo = now - (24 * 60 * 60 * 1000);

      const isLockout = recentViolations.every(row => {
        const violationDate = new Date(row[0]).getTime();
        return violationDate > twentyFourHoursAgo;
      });

      if (isLockout) {
        sendWhatsAppNotification("⚠️ Emotional Lockout Triggered: 3 violations within 24 hours. Mandatory break recommended. Discipline is the bridge between goals and accomplishment.");
      }
  }
