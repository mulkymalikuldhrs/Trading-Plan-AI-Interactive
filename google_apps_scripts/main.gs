...
const SPREADSHEET_ID = scriptProperties.getProperty('SPREADSHEET_ID') || "1I8uVUlquRPwIc_cMKr-toHZ9qHW38uDom4TYdKexaoE";
...
const USER_PHONE_NUMBER = scriptProperties.getProperty('USER_PHONE_NUMBER') || "6285322624048";
...
function analyzeAndSummarizeWeek() {
  ...
  journalData.forEach(row => {
    // Result is at index 11 (L) based on appendRow order
    if(row[11] === 'WIN') winCount++;
    const mood = row[9];
