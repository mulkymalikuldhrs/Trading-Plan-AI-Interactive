...
function getPromptTemplate(promptType) {
    const prompts = {
        ...
        'Forecast': `Berikan analisa prakiraan 7 hari untuk {{pair}} (Timeframe: {{timeframe}}).\nData COT: {{cot_summary}}\nFundamental: {{fundamental_summary}}\nStruktur Teknikal: {{technical_structure}}\nHarga Sekarang: {{current_price}}\nSupply Area: {{supply_area}}\nBeri bias dan level-level penting. Format JSON: {"bias": "", "probability": 0, "entry_zone": "", "confirmation": "", "stop_loss": "", "take_profit": ""}`
    };
