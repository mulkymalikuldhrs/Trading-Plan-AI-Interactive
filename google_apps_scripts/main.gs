...
function getPromptTemplate(promptType) {
    const prompts = {
        ...
        'Forecast': `Berikan prakiraan {{days}} hari untuk {{pair}} pada timeframe {{timeframe}}. Gunakan data pasar saat ini. Output JSON: bias, entry_zone, confirmation, stop_loss, take_profit, probability, is_tradeable.`
    };
...
function getAiMasterSummary(symbol) {
  ...
  return {
    ...analysis,
    cot_raw: marketData.cot_report // Include raw data for charts
  };
}
