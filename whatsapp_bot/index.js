...
async function handleCot(msg, args) {
    ...
    try {
        const response = await axios.post(GOOGLE_APPS_SCRIPT_URL, {
            action: 'exportToJson',
            data: { sheetName: 'Journal', apiKey: BOT_API_KEY } // Example of using exportToJson if needed, but let's assume getAiMasterSummary handles it.
        });

        // Actually COT data should come from getAiMasterSummary or a dedicated endpoint.
        // Let's call getAiMasterSummary for consistency as it includes COT.
        const resSummary = await axios.post(GOOGLE_APPS_SCRIPT_URL, {
            action: 'getAiMasterSummary',
            data: { symbol: symbol, apiKey: BOT_API_KEY }
        });

        const cot = resSummary.data.data.positional_thesis;
