...
async function handleCot(msg, args) {
    const symbol = args[0] ? args[0].toUpperCase() : 'EURUSD';
    msg.reply(`📊 Fetching Commitment of Traders (COT) data for **${symbol}**...`);
    ...
    try {
        ...
        if (response.data.status === 'success') {
            const summary = response.data.data;
            const formattedReply = `
*📊 COT Intelligence: ${symbol}*
*Institutional Bias:* ${summary.positional_thesis}

*Context:* Institutional positioning is a key driver for long-term trends.
            `;
            msg.reply(formattedReply);
        } else {
            msg.reply('GAS Error: ' + response.data.message);
        ...
        msg.reply('Sorry, I had trouble fetching COT data.');
    }
}

async function handleSetup(msg, args) {
    msg.reply('🔍 Setup validation is a complex visual process. Please use the mobile app for the full experience with TradingView integration and AI-assisted entry scoring.');
}
